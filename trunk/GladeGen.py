...
# Dave Reed
# 02/03/2004
#Corregido bug que no permitia llamar al programa desde otro directorio
#Juan Manuel Schillaci 10/15/2004
#----------------------------------------------------------------------
...
class GladeGen:
    ...
    def __init__(self, module, glade_file, class_name, directoriopath):
	...
	self.directoriopath = directoriopath
	...
	#Aca tengo que hacer que escriba el archivo en el path correspondiente
        # create file if doesn't exist
	os.chdir(self.directoriopath)
	
	#verdaderopath=self.directoriopath + "/" + self.filename
	...
	print "Imprimiendo el modulo"
	print self.directoriopath

	sys.path.append(self.directoriopath)

	print "ahora imprimiendo sys"
	print sys.path
	importa="%s" % self.module
        #self.module = __import__(self.module)
	self.module = __import__(importa)
...
def main(argv):
    ...
    if len(argv) != 5:
        print 'usage: GladeGen.py glade_file module_name class_name directorio del archivo'
        return
    
    glade_file = argv[1]
    module = argv[2]
    class_name = argv[3]
    direct_actual = argv[4]
	
    print "directorio actual", direct_actual 

    pg = GladeGen(module, glade_file, class_name, direct_actual)
